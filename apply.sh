...
original_author="farhan39"
original_name="practice_project_deployment_aws"
original_urlname="Practice-Project-Deployment-AWS"
original_description="Awesome practice_project_deployment_aws created by farhan39" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/practice_project_deployment_aws "${name}"
...
then
    ...
    echo "then run 'practice_project_deployment_aws' or 'python -m practice_project_deployment_aws'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'practice_project_deployment_aws' or 'python -m practice_project_deployment_aws' or 'poetry run practice_project_deployment_aws'"
fi
